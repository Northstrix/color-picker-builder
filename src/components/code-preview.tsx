...
// STATIC - NEVER CHANGES, NEVER RERENDERS
const STATIC_COMPONENT_SOURCE = `"use client";
...
const DEFAULT_PROPS: { [key: string]: any } = {
    ...
    hueTrackHeight: 10, hueTrackRadius: "8px", hueTrackBorderWidth: 1, hueTrackBorderColor: "transparent",
    hueThumbSize: 16, hueThumbRadius: "50%", hueThumbBorderWidth: 3, hueThumbBgDefault: "#f0f0f0",
    hueThumbBgHover: "#e5e5e5", hueThumbBgActive: "#f0f0f0", hueThumbBorderDefault: "#e5e5e5",
    hueThumbBorderHover: "#f0f0f0", hueThumbBorderActive: "#fff", badgeBorderWidth: 1, badgeBorderRadius: "50px",
    badgeFontSize: "10px", badgeFontWeight: 600, badgeIconSize: 10, badgePadding: "0.25rem 0.5rem",
...
// STATIC CodeBlock - memoized with empty deps
const StaticCodeBlock = React.memo(({ code, language }: { code: string, language: string }) => {
  return (
    <div className="relative">
      <div className="h-[32rem] w-full rounded-b-[var(--radius)] rounded-t-0 border bg-card overflow-auto">
        <SyntaxHighlighter
          language={language}
          style={vscDarkPlus}
          showLineNumbers
...
// DYNAMIC CodeBlock - only for demo code
const DynamicCodeBlock = React.memo(({ code, language }: { code: string, language: string }) => {
  return (
    <div className="relative">
      <div className="h-[32rem] w-full rounded-b-[var(--radius)] rounded-t-0 border bg-card overflow-auto">
        <SyntaxHighlighter
...
DynamicCodeBlock.displayName = "DynamicCodeBlock";

function generateDemoCode(props: ColorPickerProps, maxWidth: number) {
  let propsString = "";

  for (const key in props) {
    const pKey = key as keyof ColorPickerProps;
    const value = props[pKey];
    const defaultValue = DEFAULT_PROPS[pKey];
...
export default function MyColorPicker() {
  const [color, setColor] = useState("${props.value || "#06B5EF"}");
  ...
  return (
    <div style={{ maxWidth: "${maxWidth}px", width: "100%" }}>
      <ColorPicker
        value={color}
        onValueChange={handleColorChange}${propsString}
      />
    </div>
  );
}`;
...
export default function CodePreview({ props, maxWidth }: CodePreviewProps) {
  // ONLY demo code recomputes
  const demoCode = useMemo(() => generateDemoCode(props, maxWidth), [props, maxWidth]);

  return (
    <div className="w-full space-y-4">
      {/* DYNAMIC Demo Code - updates when props change */}
      <div className="w-full">
        <div className="px-3 py-2.5 text-[0.825rem] rounded-t-[var(--radius)] rounded-b-0 font-medium text-muted-foreground border border-b-0 bg-card/50">
          Demo Code (JSX)
        </div>
        <DynamicCodeBlock code={demoCode} language="jsx" />
      </div>
      
      {/* STATIC Component Source - NEVER updates */}
      <div className="w-full">
        <div className="px-3 py-2.5 text-[0.825rem] rounded-t-[var(--radius)] rounded-b-0 font-medium text-muted-foreground border border-b-0 bg-card/50">
          Component Source
        </div>
        <StaticCodeBlock code={STATIC_COMPONENT_SOURCE} language="tsx" />
      </div>
    </div>
  );
}
